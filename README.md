...
Положить в неё файл `generate.py`.

Сгенерировать стенд из трёх nginx:
...
Для стенда с большим количеством nginx можно проверить любой другой сервис:

```bash
docker compose exec nginx-1 sh -c 'nslookup nginx-5 127.0.0.11'
```

Если имя резолвится, значит nginx может обратиться к другому nginx по имени вида:
...
Приложение вернёт JSON с заголовками. Важные поля:
...
Значение IP-адресов может отличаться в зависимости от Docker network на конкретной машине.

Важно не конкретное значение IP, а структура цепочки:

```text
IP пользователя, IP первого nginx, IP второго nginx, ...
```

### 2. Проверка разных маршрутов

Выполнить несколько запросов подряд:

```bash
for i in $(seq 1 10); do
  curl -s http://localhost:8080 | grep -E 'x_forwarded_for|x_visited_nginx'
  echo
done
```

Ожидаемо маршруты будут отличаться, например:

```text
"x_forwarded_for": "172.18.0.1, 172.18.0.5"
"x_visited_nginx": "nginx-1"

"x_forwarded_for": "172.18.0.1, 172.18.0.6, 172.18.0.8"
"x_visited_nginx": "nginx-2,nginx-4"

"x_forwarded_for": "172.18.0.1, 172.18.0.7, 172.18.0.5, 172.18.0.6"
"x_visited_nginx": "nginx-3,nginx-1,nginx-2"
```

Это подтверждает, что запрос может пройти как через один nginx, так и через цепочку из нескольких nginx.

### 3. Проверка защиты от поддельного X-Forwarded-For
...
Ожидаемо в поле `x_forwarded_for` не должно быть адресов:
...
Корректный результат будет выглядеть примерно так:

```json
{
  "x_forwarded_for": "172.18.0.1, 172.18.0.5, 172.18.0.7",
  "x_visited_nginx": "nginx-1,nginx-3"
}
```
...
### 4. Проверка защиты от поддельного служебного X-Visited-Nginx

Пользователь пытается повлиять на внутреннюю маршрутизацию:

```bash
curl -s \
  -H "X-Forwarded-For: 8.8.8.8, 1.1.1.1" \
  -H "X-Visited-Nginx: nginx-1,nginx-2,nginx-3" \
  http://localhost:8080
```

Ожидаемо первый nginx после балансировщика начнёт служебный заголовок заново, поэтому пользовательский `X-Visited-Nginx` тоже не должен влиять на построение маршрута.

В ответе не должно быть поддельных IP:
...
### 5. Проверка, что наружу опубликован только балансировщик
...
Ожидаемо наружу должен быть опубликован только порт:

```text
localhost:8080 → lb:80
```

Сервисы `nginx-1 ... nginx-N` не должны публиковать порты наружу. Они доступны только внутри Docker network.
